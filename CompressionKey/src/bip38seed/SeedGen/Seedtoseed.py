...
def bytes_to_seed_phrase(encrypted_bytes):
    indices = [b % WORD_LIST_LENGTH for b in encrypted_bytes]  # Convert bytes to indices
...
def encrypt_button_click():
    ...
    try:
        ...
        encrypted_seed_phrase_entry.delete("1.0", tk.END)
        encrypted_seed_phrase_entry.insert(tk.END, encrypted_seed)
    except Exception as e:
        messagebox.showerror("Error", str(e))

# Function to handle decryption button click
def decrypt_button_click():
    encrypted_seed = encrypted_seed_phrase_entry.get("1.0", "end-1c").strip()
    password = decrypt_password_entry.get()

    if not encrypted_seed or not password:
        messagebox.showerror("Error", "Please enter both an encrypted seed phrase and a password.")
        return

    try:
        decrypted_seed = decrypt_seed_phrase(encrypted_seed, password)
        decrypted_seed_phrase_entry.delete("1.0", tk.END)
        decrypted_seed_phrase_entry.insert(tk.END, decrypted_seed)
    except Exception as e:
        messagebox.showerror("Error", str(e))

# GUI Setup
root = tk.Tk()
root.title("Seed Phrase Encryptor/Decryptor")

# Seed phrase input
tk.Label(root, text="Seed Phrase:").pack()
seed_phrase_entry = tk.Text(root, height=3, width=50)
seed_phrase_entry.pack()

# Password for encryption
tk.Label(root, text="Password for Encryption:").pack()
...
# Encrypted Seed Phrase Output
tk.Label(root, text="Encrypted Seed Phrase:").pack()
encrypted_seed_phrase_entry = tk.Text(root, height=3, width=50)
encrypted_seed_phrase_entry.pack()

# Password for decryption
tk.Label(root, text="Password for Decryption:").pack()
...
# Decrypted Seed Phrase Output
tk.Label(root, text="Decrypted Seed Phrase:").pack()
decrypted_seed_phrase_entry = tk.Text(root, height=3, width=50)
decrypted_seed_phrase_entry.pack()
